...
contract RockPaperScissors {
    ...
    // signature = keccak256(move + salt)
    function commitMove(bytes32 signedMove) public payable
        isValidPlayer
        ongoingGame
    ...
    // Here a player chooses to reveal their move
    // If it is an invalid reveal, we say they are cheating
    // Regardless of validity, we set a 'firstReveal' time counter
    // to force the other player to eventually reveal, otherwise
    // they may choose to not play at all
    // This is the 3rd stage of the game
    function revealMove(string move, string salt) public payable
        isValidPlayer
        ongoingGame
        bothPlayersHaveRegistered
        bothPlayersHaveCommitted
    {
        if (msg.sender == p1 && !p1Revealed) {
            p1Revealed = true;
            // It is up to the player to responsibly pass in a valid move
            p1Move = move;
            // Check validity of signature
            if (p1Commitment != keccak256(move.toSlice().concat(salt.toSlice()))) {
                p1Cheated = true;
            }
            // Check move validity
            if (!isValidMove(move)) {
            ...
            // If this is the first reveal, set firstReveal counter
            if (keccak256(p2Move) != keccak256("")) {
                firstReveal = now;
            }
        } else if (msg.sender == p2 && !p2Revealed) {
            p2Revealed = true;
            // It is up to the player to responsibly pass in a valid move
            p2Move = move;
            // Check validity of signature
            if (p2Commitment != keccak256(move.toSlice().concat(salt.toSlice()))) {
                p2Cheated = true;
            }
            // Check move validity
            if (!isValidMove(move)) {
    ...
    function payWinnings() private {
        ...
        if (p1Cheated && p2Cheated) {
            // Split the balance between the 2...grudgingly
            p1.transfer(this.balance / 2);
            p2.transfer(this.balance);
            return;
        } else if (p1Cheated) {
            // P2 hasn't cheated, so wins
            p2.transfer(this.balance);
            return;
        } else if (p2Cheated) {
            // P1 hasn't cheated, so wins
    ...
    // 1 = P1 winner
    // 2 = P2 winner
    function decideWinner() private view returns (uint winner) {
        ...
        if (keccak256(p1Move) == keccak256("rock")) {
            if (keccak256(p2Move) == keccak256("paper")) {
                return 2;
            } else if (keccak256(p2Move) == keccak256("scissors")) {
                return 1;
            }
        } else if (keccak256(p1Move) == keccak256("paper")) {
            if (keccak256(p2Move) == keccak256("rock")) {
                return 1;
            } else if (keccak256(p2Move) == keccak256("scissors")) {
                return 2;
            }
        } else if (keccak256(p1Move) == keccak256("scissors")) {
            if (keccak256(p2Move) == keccak256("paper")) {
                return 1;
            } else if (keccak256(p2Move) == keccak256("rock")) {
                return 2;
            }
        }
    }
